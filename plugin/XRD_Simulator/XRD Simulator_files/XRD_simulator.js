...
const do_when_done = (max, str_id, images_arr) => {
    ...
    switch (promise_fullfilled_num) {
        case 1:
            i_promise_file(1, `XRD%20Simulator_files/XRD_cartesian_plane_`, `js_script`, sprites[promise_fullfilled_num], `js`);
...
// =======================  END END  ==================== //
i_promise_file(1, `XRD%20Simulator_files/XRD_radiometer_`, `js_script`, sprites[promise_fullfilled_num], `js`);
// =======================  END END  ==================== //
// ======================= START INSTRUCTIONS ======================== //
...
$(function () {
    ...
    $("#time-step").selectmenu({
        select: function (event, data) {
            ...
            if (XRD_step_num != 7) $("#btn-start-scan").toggleClass("current-control", false);
            XRD_step_num = 7;
    ...
    $("#specimen-button").toggleClass("current-control", true);
    // =======================  END END DROP DOWN  ==================== //
    triggerResize();
});
// ======================= END ======================== //
...
$('.btn').click(function (e) {
    ...
    switch (e.target.id) {
        case "btn-doors":
            ...
            $("#btn-doors").toggleClass("current-control", false);
            break;
        case "btn-start-scan":
    ...
    close_doors
        ...
        .add({
            ...
            complete: function (anim) {
                if (!bool_first_time) {
                    XRD_step_num = 5;
                    $("#bottom-stats").toggleClass("current-control", true);
                }
                next_step();
            }
        }, '-=1000');
...
$('#btn-start-dw').click(function (e) {
    $("#bottom-stats").toggleClass("current-control", false);
    $("#step").toggleClass("current-control-button", false);
    if (start_angle > 5) {
...
//         "level_name": "XRD simulator begins"
//     });       
//     gtag('event', 'page_view', {
//         'page_title': `XRD simulator`,
//         'content_group': `XRD module`,
...
function saveXRDIMG(the_svg) {
    ...
    lnk.download = `Diffractogram of ${specimen_title}, from the XRD simulator - www.myscope.training.jpg`;
...
// ======================= END END END SITEMAP ================== //
    // Function to check screen size and toggle class
    function toggleAlertOnPhoneSize() {
        if ($(window).width() <= 1023) { 
            $('#modal-resize').toggleClass("totally-hidden", false);
            $('#resize-window.modal-window').css('width', '80%');
        } else {
            $('#modal-resize').toggleClass("totally-hidden", true);
            $('#resize-window.modal-window').css('width', '50%');
        }
    }

    // Initial check when the document is ready
    toggleAlertOnPhoneSize();
